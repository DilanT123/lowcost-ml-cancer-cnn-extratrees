...
BATCH_SIZE = 16  # Reducido para manejar posibles problemas de memoria con mayor resolución
NUM_WORKERS = 2   # Reducido para mayor estabilidad
PREFETCH_FACTOR = 1  # Reducido para problemas de memoria
IMAGE_SIZE = 224 # Nueva resolución de imagen
NUM_EPOCHS = 50 # Aumentar el número de épocas
...
# --- Arquitectura CNN Optimizada (Reemplazada por ResNet) ---
...
def run_cnn_training():
    try:
        ...
        # 6. Modelo (Usando ResNet18 pre-entrenado)
        print("\n🧠 Inicializando modelo (ResNet18 pre-entrenado)...")
        model = models.resnet18(pretrained=True)
        num_ftrs = model.fc.in_features
        model.fc = nn.Linear(num_ftrs, len(expected_classes))
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5) # Aumentar paciencia
